...
public class FireABeam : MonoBehaviour {
		...
		void Start () {
			...
			playerRg = GameObject.FindGameObjectWithTag ("Player").GetComponentInParent<Rigidbody> ();
			playerScript = GameObject.FindGameObjectWithTag ("Player").GetComponentInParent<RigidbodyFirstPersonController> ();
		...
		void Update () {
			...
			if(hitPeople!=null){
				...
				if (hitPeople.name == "Rose"&&onlyCheckOnce) {
					//call function from rose
					hitPeople.gameObject.SendMessage ("EnableEmoji_Heart");
					FindHer ();
				}
	...
	//			lRender.dest = desPoint;
				...
				if (Vector3.Distance (desPoint, transform.position) < minRangeToDrop) {
					...
					if (!playerScript.Grounded) {//add force to land when on air
						playerRg.AddForce (Vector3.up * upForceScaler, ForceMode.Impulse);
					}
		...
		IEnumerator GoToNextScene(){
			gameObject.BroadcastMessage ("ShowTheFlower");
